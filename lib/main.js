...
// Launch window
// Fetch latest updates in case we switch to mirobot network
// On Connect:
//   - Fetch the current installed versions
// Update and re-check
  ...
  mainWindow.webContents.on('did-finish-load', function() {
    ...
    ipc.on('connect', function(event, ip) {
      ...
      mirobot.on('connect', function(){
        ...
        versions.arduino = mirobot.arduinoVersion;
        sendVersions();
        mirobot.getUIVersion(function(err, v){
          if(!err){
            ...
            sendStatus("Error fetching UI version. Please restart Mirobot and try again.");
            console.log(err.msg);
          }
        });
      });
    ...
    ipc.on('upgradeArduino', function(){
      ...
      mirobot.updateFirmware(arduinoVersion.asset, function(succ){
        if(succ){
